...
Verificação obrigatória pedida ao agente confirmou, por leitura real do código (não suposição): `useSessions.ts`, `useSession.ts` e `useSeatMap.ts` chamam `apiClient` sem header de `Authorization` — os outros 8 pontos do frontend que usam auth passam o header explicitamente, esses 3 não. Consequência real, já confirmada: `useMySessions.ts` (painel do organizador, D50), que filtra client-side por `organizerId`, vai parar de mostrar os próprios rascunhos assim que este backend for pra produção — a chamada anônima só recebe sessões `published: true`. Regressão real na jornada do organizador (D49, `organizer-journey.e2e-spec.ts`) já foi encontrada e corrigida no lado backend do teste; falta o equivalente no frontend real. **Risco #6 não é considerado fechado até essa correção de frontend entrar na mesma branch** — merge para `develop` fica pendente disso.
