...
**Fechamento (24/08):** backend completo — `POST /auth/refresh` (rotação de refresh token a cada uso, `User.refreshTokenHash` que era código morto agora escrito de verdade) e `POST /auth/logout` real (invalidação server-side, escopo expandido durante a própria tarefa ao descobrir que logout nunca existira no backend — sem isso, refresh confiando no hash seria pior que a ausência original de refresh). Frontend: refresh silencioso em `api-client.ts` com deduplicação de chamadas 401 concorrentes (uma única chamada de refresh compartilhada, não uma por requisição — provado com teste unitário isolado disparando 5 401s verdadeiramente concorrentes), logout aguardando confirmação do backend antes de limpar estado local. Verificação A/B completa contra o `useAuth.tsx` pré-tarefa (mesmo método já usado numa investigação anterior nesse arquivo) confirmou ausência de regressão no caminho feliz dos 4 papéis. Risco #11 fechado. Backlog do Sprint 5 sem pendências conhecidas restantes.

## Ciclo 10 (continuação) — Achado real de produção, ausente em teste local (24/08)

**D59 — Cookie de refresh (D58) não funciona em produção: `SameSite=Lax` não é enviado em requisições cross-site (fetch/XHR), só em navegação top-level**
Confirmado em teste real contra produção (Vercel + Railway, domínios genuinamente diferentes — D13 já assumiu esse trade-off de infraestrutura, mas sem prever esta consequência específica): `Set-Cookie: refreshToken=...; HttpOnly; Secure; SameSite=Lax` chega na resposta de `/auth/login`, mas o navegador nunca o anexa de volta em chamadas subsequentes (`context.cookies()` vazio) — `SameSite=Lax` restringe cookies de contexto cross-site a navegação de página inteira, nunca a `fetch`/XHR, que é como o frontend se comunica com a API. Em `localhost`, os dois serviços compartilham host (só porta muda), então o problema nunca apareceu em nenhuma validação local anterior (D58 fechado "corretamente" com base em teste que não cobria esse cenário).

Consequência observada: sessão nunca sobrevive além do access token (15min) em produção — todo refresh cai em `401` limpo (fallback correto, comportamento gracioso, sem tela zumbi), mas a funcionalidade real (renovação silenciosa) nunca funciona. Logout real também é afetado: `POST /auth/logout` é chamado, mas sem cookie, o backend não identifica de quem é a sessão — `refreshTokenHash` nunca é zerado por essa via, fica órfão até expirar sozinho em 7 dias.

Correção: `SameSite=None; Secure` no cookie de refresh (troca de `Lax`) — requisito mínimo da especificação pra cookie funcional em contexto cross-site via fetch, não workaround. `Secure` já era necessário e ambos os domínios já são HTTPS (Vercel + Railway), sem custo adicional.

**Nota técnica pós-correção (25/08):** re-teste em produção mostrou o cookie ainda ausente em cenário cross-site real via automação (Playwright), apesar do backend confirmado rodando o código corrigido (log de boot do Railway mostra rotas `/auth/refresh`/`/auth/logout` mapeadas, deploy ativo desde antes do teste). Dois testes de controle isolaram a causa: mesma sequência login→refresh **same-site** funciona (`201`, token novo); **cross-site** falha (`401`, cookie ausente) — ou seja, o mecanismo de rotação em si está correto, o que falha é especificamente a entrega do cookie nesse contexto de teste. Hipótese mais provável, não uma falha de código: contextos de navegador efêmeros/automatizados (como os que Playwright cria por padrão) frequentemente herdam comportamento de bloqueio de cookies de terceiros equivalente ao modo anônimo — que, diferente do Chrome padrão (que mantém cookies de terceiros habilitados por escolha do usuário desde a reversão do plano de depreciação em 2024), bloqueia por padrão. Um usuário real navegando com perfil normal do Chrome provavelmente não encontra este problema. Sem ação adicional de código — a única forma de eliminar esse risco por completo seria uma mudança de arquitetura maior (proxy da API pelo mesmo domínio do frontend via rewrite da Vercel, tornando o cookie first-party do ponto de vista do navegador), não justificada neste momento.
