...
- [x] **Bloqueador do deploy real (Sprint 5) resolvido**: `backend/Dockerfile` agora copia `src/prisma/` (schema+migrations+seed.ts) para o runtime — ver risco #8.
- [ ] Backlog (não bloqueia Sprint 3, mas não é o mesmo bloqueador acima): D35 (retry com backoff no Prisma), `/auth/login` sem `ZodValidationPipe`, D40 (filtro de `published`/dono nos 3 endpoints REST de sessions), risco #9 (`ts-node`/seed quebrado na imagem de runtime atual).
...
8. ~~`backend/Dockerfile` não copia `src/prisma/` (schema + migrations) para o estágio final de runtime~~ — **resolvido**: `Dockerfile` agora copia `backend/src/prisma` (schema.prisma + migrations/ + seed.ts) do estágio `builder` para o `runner`. Reproduzido o erro original antes da correção (`docker build` + `prisma migrate deploy` dentro do container: `Could not load schema from src/prisma/schema.prisma ... file or directory not found`, contra o Postgres de dev local na porta 5434); após a correção, `prisma migrate deploy` dentro do container encontra o schema e as migrations corretamente (`No pending migrations to apply` — banco já estava migrado por uso anterior). Build multistage e contexto na raiz (D34) preservados sem alteração. Boot normal validado dentro do container (via `docker exec`, sem SSH manual): app sobe limpo com Postgres acessível e `/docs` responde 200.
9. **Novo achado, não corrigido nesta tarefa (fora de escopo — fix era só de packaging do Dockerfile, sem tocar `src/`): `ts-node` (10.9.2) quebra dentro da imagem `node:20-alpine` atual (Node 20.20.2) ao rodar `backend/src/prisma/seed.ts`** — falha com `TypeError: Unknown file extension ".ts"` (ou, forçando `--no-experimental-detect-module`, `SyntaxError: Cannot use import statement outside a module`). Reproduzido também com um arquivo `.ts` trivial fora do projeto — não é problema de path/packaging, é incompatibilidade `ts-node`/detecção de módulo do Node nesta versão de patch da imagem base. Efeito prático: `prisma migrate deploy` funciona normalmente dentro do container (risco #8 resolvido), mas `pnpm seed`/`ts-node src/prisma/seed.ts` não roda como está. Prioridade antes do Sprint 5 se o seed for necessário no deploy real — requer decisão do Backend Agent (upgrade de `ts-node`, ou compilar o seed para JS junto do `nest build`, ou trocar por script Node puro).
