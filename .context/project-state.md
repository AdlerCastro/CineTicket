...
11. ~~`GET /sessions`/`GET /sessions/:id` sem dado de filme~~ — **resolvido (D44, 24/08)**: relação `movie` incluída via Prisma `include`. `GET /sessions/:id/seats` deliberadamente fora do escopo (é DTO de mapa de assentos, não de sessão).
